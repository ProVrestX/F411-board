...
#ifdef ST7735_CS_PORT
    #define ST7735_Select()  (ST7735_CS_GPIO_Port->BSRR = ((uint32_t)ST7735_CS_Pin << 16))
    #define ST7735_Unselect() (ST7735_CS_GPIO_Port->BSRR = ST7735_CS_Pin)
#else
    #define ST7735_Select()
    #define ST7735_Unselect()
#endif
...
#endif
static uint8_t min_y = 0, max_y = 0;

#if FRAME_BUFFER
// массив буфер кадра
	static uint16_t buff_frame[ST7735_WIDTH*ST7735_HEIGHT] = { 0x0000, };
#endif
	...
	//-- если захотим переделать под HAL ------------------
			...
			if( buff_size <= 0xFFFF ){
				...
				while( buff_size > 0xFFFF ){
					ST7735_dma_state = 1;
					HAL_SPI_Transmit_DMA(&ST7735_SPI_HAL, buff, 0xFFFF);
					while(ST7735_dma_state);
					buff_size-=0xFFFF;
					buff+=0xFFFF;
				}
...
//==============================================================================
    ...
    void ST7735_Update(void){
        ...
        uint16_t *buff_frame_tmp = buff_frame + ST7735_Width*min_y;
        uint8_t y_range = max_y - min_y;

		ST7735_SetWindow(0, min_y, ST7735_Width-1, max_y-1);

		ST7735_Select();

		ST7735_SendDataMASS((uint8_t*)buff_frame_tmp, sizeof(uint16_t)*ST7735_Width*y_range);
