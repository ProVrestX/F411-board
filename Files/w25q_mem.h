...
W25Q_STATE W25Q_BlockReadOnly(uint32_t Addr, uint8_t enable);			///< Individual block/sector read-only lock
W25Q_STATE W25Q_BlockReadOnlyCheck(uint8_t *state, uint32_t Addr);	///< Check block's/sector's read-only lock status
W25Q_STATE W25Q_GlobalReadOnly(uint8_t enable);		///< Set read-only param to all chip

W25Q_STATE W25Q_SwReset(uint8_t force);	///< Software reset

W25Q_STATE W25Q_EnableMemoryMapped(void);
W25Q_STATE W25Q_DisableMemoryMapped(void);
extern volatile uint8_t qspi_dma_complete;
