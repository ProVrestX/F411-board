 ...
 * @{
 */
W25Q_STATE W25Q_WriteEnable(uint8_t enable); 	///< Toggle WOL bit
W25Q_STATE W25Q_EnableQSPI(uint8_t enable);	///< Toggle QE bit
W25Q_STATE W25Q_Enter4ByteMode(uint8_t enable); 	///< Toggle ADS bit
...
//	state = W25Q_ReadID(&id);
	state = W25Q_ReadJEDECID(id);

	uint8_t msg[32] = {0};
	uint8_t msg_len = sprintf((char*)msg, "ID: 0x%02X%02X%02X\r\n", id[0], id[1], id[2]);
	USB_Transmit(msg, msg_len);
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_BlockReadOnly(uint32_t Addr, uint8_t enable) {
	return W25Q_PARAM_ERR;
}
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_BlockReadOnlyCheck(uint8_t *state, uint32_t Addr) {
	return W25Q_PARAM_ERR;
}
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_GlobalReadOnly(uint8_t enable){
	return W25Q_PARAM_ERR;
}
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_SwReset(uint8_t force) {
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_WriteEnable(uint8_t enable) {
	QSPI_CommandTypeDef com;
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_Enter4ByteMode(uint8_t enable) {
...
// ─── Memory Mapped Mode (чтение как из RAM по адресу 0x90000000) ────────────
W25Q_STATE W25Q_EnableMemoryMapped(void) {
    HAL_QSPI_Abort(&hqspi1);

    QSPI_CommandTypeDef cmd = {0};
    cmd.Instruction = W25Q_FAST_READ_QUAD_OUT;
    ...
    if (HAL_QSPI_MemoryMapped(&hqspi1, &cmd, &mem_mapped_cfg) != HAL_OK)
	{
		return W25Q_SPI_ERR;
	}
...
W25Q_STATE W25Q_DisableMemoryMapped(void) {
	return HAL_QSPI_Abort(&hqspi1);
}

volatile uint8_t qspi_dma_complete = 0;
void HAL_QSPI_RxCpltCallback(QSPI_HandleTypeDef *hqspi)
{
    if (hqspi->Instance == QUADSPI) {
        qspi_dma_complete = 1;  // Сигнализируем о завершении
    }
}
W25Q_STATE W25Q_ReadData_DMA(uint32_t addr, uint8_t *buf, uint32_t size)
{
    // 1. Выход из Memory Mapped Mode, если он активен
//    HAL_QSPI_Abort(&hqspi1);

    // 2. Сбрасываем флаг
    qspi_dma_complete = 0;

    // 3. Настраиваем команду (Quad Read, 4 линии)
    QSPI_CommandTypeDef cmd = {0};
    cmd.Instruction       = 0x6B;  // Fast Read Quad I/O
    cmd.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
    cmd.AddressMode       = QSPI_ADDRESS_1_LINE;
    cmd.AddressSize       = QSPI_ADDRESS_24_BITS;
    cmd.Address           = addr;
    cmd.DataMode          = QSPI_DATA_4_LINES;  // 🔑 Quad Output
    cmd.DummyCycles       = 8;                   // 🔑 Обязательно для 0x6B
    cmd.NbData            = size;
    cmd.DdrMode           = QSPI_DDR_MODE_DISABLE;
    cmd.SIOOMode          = QSPI_SIOO_INST_EVERY_CMD;

    // 4. Отправляем команду
    if (HAL_QSPI_Command(&hqspi1, &cmd, HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
//    	USB_Transmit("CMD err\r\n", 9);
        return W25Q_SPI_ERR;
    }

    // 5. Запускаем прием через DMA
    if (HAL_QSPI_Receive_DMA(&hqspi1, buf) != HAL_OK) {
//    	USB_Transmit("DMA err\r\n", 9);
        return W25Q_SPI_ERR;
    }

    return W25Q_OK;  // Функция вернулась сразу, данные ещё не в буфере!
}
